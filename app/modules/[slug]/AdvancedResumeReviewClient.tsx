...
import { CalendarCheck, Check, Clock3, FileText, RefreshCw, Save, Sparkles, Upload } from "lucide-react";
import { GhostButton } from "@/components/Prototype";
import { usePrototypeState } from "@/components/PrototypeState";

const moduleSlug = "advanced-resume-review";
const storageKey = "aggies-lead:junior:advanced-resume-review";

const assessmentItems = [
  "Resume updated within the last 6 months",
  "Current major listed",
  "Expected graduation date listed",
  "Student-athlete experience included",
  "Leadership experiences included",
  "Community service included",
  "Work experience included",
  "Internship experience included (if applicable)",
...
const initialState: AdvancedResumeState = {
  assessment: [],
  readiness: "",
...
export function AdvancedResumeReviewClient() {
  const { completeModule } = usePrototypeState();
  const [form, setForm] = useState<AdvancedResumeState>(initialState);
  ...
  const completion = useMemo(() => {
    const tasks = [
      form.assessment.length > 0,
      form.scheduledReview || form.uploadedResume,
  ...
  function toggleAssessment(item: string) {
    setSavedMessage("");
    setForm((current) => ({
      ...current,
      assessment: current.assessment.includes(item)
        ? current.assessment.filter((value) => value !== item)
        : [...current.assessment, item],
    }));
  }

  function updateField<K extends keyof AdvancedResumeState>(field: K, value: AdvancedResumeState[K]) {
    setSavedMessage("");
  ...
  function markComplete() {
    ...
    setSavedMessage("Advanced Resume Review completed.");
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-lg leading-8 text-aggie-light/78">
              As you gain leadership experience, internships, work experience, and involvement outside of athletics,
              your resume should evolve. This module will help you strengthen your resume and prepare for internships,
              graduate school, and future employment opportunities.
            </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
        <main className="space-y-6">
          <Card title="Resume Self-Assessment" icon={<FileText className="h-6 w-6" />}>
            <div className="grid gap-3 md:grid-cols-2">
              {assessmentItems.map((item) => (
                <ToggleCard key={item} selected={form.assessment.includes(item)} label={item} onClick={() => toggleAssessment(item)} />
              ))}
            </div>
          </Card>

          <Card title="Common Resume Mistakes" icon={<Sparkles className="h-6 w-6" />}>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {commonMistakes.map((mistake) => (
                <article key={mistake} className="rounded-lg border border-aggie-silver/15 bg-white/[0.045] p-5">
                  <p className="text-sm font-black leading-6 text-white">{mistake}</p>
                </article>
              ))}
            </div>
          </Card>

          <Card title="Strong Bullet Points" icon={<Check className="h-6 w-6" />}>
            <div className="grid gap-4 md:grid-cols-2">
              <BulletExample weak="Worked summer camp." strong="Led daily activities for 50+ campers while developing leadership, communication, and problem-solving skills." />
              <BulletExample weak="Member of SAAC." strong="Collaborated with athletic department staff and student-athletes to improve engagement initiatives and support community outreach projects." />
            </div>
          </Card>

          <Card title="Resume Review Resources" icon={<CalendarCheck className="h-6 w-6" />}>
            <div className="grid gap-3 sm:grid-cols-2">
              <ActionButton onClick={() => updateField("scheduledReview", true)} icon={<CalendarCheck className="h-4 w-4" />} primary={form.scheduledReview}>
                Schedule Resume Review
              </ActionButton>
              <ActionButton onClick={() => updateField("uploadedResume", true)} icon={<Upload className="h-4 w-4" />} primary={form.uploadedResume}>
                Upload Resume for Feedback
              </ActionButton>
            </div>
            <p className="mt-4 text-sm font-semibold leading-6 text-aggie-light/72">
              These buttons are placeholders for prototype testing and can connect to scheduling or upload tools later.
            </p>
          </Card>

          <Card title="Resume Readiness Score" icon={<Sparkles className="h-6 w-6" />}>
        ...
        <aside className="space-y-6">
          ...
          <section className="card-surface rounded-lg p-5">
            ...
            <div className="mt-4 space-y-3 text-sm font-bold text-aggie-light/78">
              <Snapshot label="Checklist" value={`${form.assessment.length}/${assessmentItems.length}`} />
              <Snapshot label="Review Scheduled" value={form.scheduledReview ? "Yes" : "No"} />
              <Snapshot label="Resume Uploaded" value={form.uploadedResume ? "Yes" : "No"} />
...
function ProgressPanel({ completed, percent, status, total }: { completed: number; percent: number; status: string; total: number }) {
  return (
    <div className="w-full rounded-lg border border-aggie-silver/15 bg-white/[0.045] p-5 lg:max-w-sm">
      ...
      <p className="mt-3 text-sm font-medium text-aggie-light/72">{completed} of {total} resume steps complete</p>
    </div>
  );
}
...
function BulletExample({ weak, strong }: { weak: string; strong: string }) {
  return (
    <article className="rounded-lg border border-aggie-silver/15 bg-white/[0.045] p-5">
      <p className="text-sm font-black uppercase tracking-[0.18em] text-aggie-silver">Weak</p>
      <p className="mt-2 font-bold text-aggie-light/76">{weak}</p>
      <p className="mt-5 text-sm font-black uppercase tracking-[0.18em] text-aggie-silver">Strong</p>
      <p className="mt-2 font-bold leading-7 text-white">{strong}</p>
    </article>
  );
}
