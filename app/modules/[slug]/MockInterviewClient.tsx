...
import { CalendarCheck, Check, ChevronDown, Clock3, MessageSquareText, RefreshCw, Save, Sparkles } from "lucide-react";
import { GhostButton } from "@/components/Prototype";
import { usePrototypeState } from "@/components/PrototypeState";

const moduleSlug = "mock-interview";
const storageKey = "aggies-lead:junior:mock-interview";
...
type MockInterviewState = {
  ...
  scheduledMockInterview: boolean;
...
export function MockInterviewClient() {
  ...
  const [form, setForm] = useState<MockInterviewState>(initialState);
  ...
  const completion = useMemo(() => {
    const tasks = [
      form.prepChecklist.length > 0,
      form.openQuestions.length > 0,
      form.scheduledMockInterview,
  ...
  function toggleList(field: "prepChecklist" | "openQuestions", value: string) {
    setSavedMessage("");
    setForm((current) => {
  ...
  function updateField<K extends keyof MockInterviewState>(field: K, value: MockInterviewState[K]) {
    setSavedMessage("");
  ...
  function markComplete() {
    ...
    setSavedMessage("Mock Interview completed.");
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-lg leading-8 text-aggie-light/78">
              Interviewing is a skill that improves with practice. This module will help you prepare for internships,
              graduate school interviews, and future employment opportunities.
            </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
        <main className="space-y-6">
          <Card title="Interview Preparation Checklist" icon={<Check className="h-6 w-6" />}>
            <div className="grid gap-3 md:grid-cols-2">
              {prepItems.map((item) => (
                <ToggleCard key={item} selected={form.prepChecklist.includes(item)} label={item} onClick={() => toggleList("prepChecklist", item)} />
              ))}
            </div>
          </Card>

          <Card title="Common Interview Questions" icon={<MessageSquareText className="h-6 w-6" />}>
                ...
                return (
                  ...
                  >
                    <span className="flex items-center justify-between gap-4">
                      <span className="font-black text-white">Question: {question}</span>
                    ...
                    {open && (
                      <span className="mt-3 block text-sm font-semibold leading-6 text-aggie-light/76">
                        Practice a concise answer and include a specific example when possible.
                      </span>
                    )}
                  </button>
                );
          ...
          <Card title="STAR Method" icon={<Sparkles className="h-6 w-6" />}>
            ...
            <div className="mt-5 rounded-lg border border-aggie-chrome/30 bg-aggie-chrome/10 p-5">
              ...
              <p className="mt-3 leading-7 text-aggie-light/84">
                During a demanding travel week, I had two exams and a conference competition. I created a study
                schedule, communicated with my professors, and coordinated with academic support. As a result, I stayed
                on track academically while meeting my team responsibilities.
              </p>
            </div>
          </Card>

          <Card title="Questions You Should Ask" icon={<MessageSquareText className="h-6 w-6" />}>
            <CheckGrid items={questionsToAsk} />
          </Card>

          <Card title="Schedule a Mock Interview" icon={<CalendarCheck className="h-6 w-6" />}>
            <button
              type="button"
              onClick={() => updateField("scheduledMockInterview", true)}
          ...
          <Card title="Interview Confidence Check" icon={<Sparkles className="h-6 w-6" />}>
            <p className="leading-7 text-aggie-light/80">Rate your confidence:</p>
          ...
          <Card title="Module Completion" icon={<Check className="h-6 w-6" />}>
            ...
            {form.moduleStatus === "Completed" && (
              <p className="mt-5 rounded-lg border border-aggie-chrome/30 bg-white/[0.08] p-4 text-sm font-bold leading-6 text-aggie-ice">
                Mock Interview complete. Keep practicing so your answers feel natural and specific.
              </p>
            )}
          </Card>
        </main>
        ...
        <aside className="space-y-6">
          ...
          <section className="card-surface rounded-lg p-5">
            ...
            <div className="mt-4 space-y-3 text-sm font-bold text-aggie-light/78">
              <Snapshot label="Preparation" value={`${form.prepChecklist.length}/${prepItems.length}`} />
              <Snapshot label="Questions Opened" value={`${form.openQuestions.length}/${commonQuestions.length}`} />
              <Snapshot label="Mock Scheduled" value={form.scheduledMockInterview ? "Yes" : "No"} />
...
function ProgressPanel({ completed, percent, status, total }: { completed: number; percent: number; status: string; total: number }) {
  return (
    <div className="w-full rounded-lg border border-aggie-silver/15 bg-white/[0.045] p-5 lg:max-w-sm">
      ...
      <p className="mt-3 text-sm font-medium text-aggie-light/72">{completed} of {total} interview steps complete</p>
    </div>
  );
}
